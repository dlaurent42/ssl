	...
	char *message, unsigned long long len, unsigned long long padded_len)
{
	...
	new[padded_len - 3] = (8 * len >> 40) & 0xFF;
	new[padded_len - 4] = (8 * len >> 32) & 0xFF;
	new[padded_len - 5] = (8 * len >> 24) & 0xFF;
	new[padded_len - 6] = (8 * len >> 16) & 0xFF;
	new[padded_len - 7] = (8 * len >> 8) & 0xFF;
	new[padded_len - 8] = (8 * len) & 0xFF;
	return (new);
}
